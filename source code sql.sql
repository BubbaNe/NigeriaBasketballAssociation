...
INSERT INTO TrinhDoChuyenMon VALUES
('TD_SC',  N'Sơ cấp'),
('TD_TC',  N'Trung cấp'),
('TD_CD',  N'Cao đẳng'),
('TD_DH',  N'Đại học'),
...
CREATE TABLE TrinhDoLLCT (
    MaLLCT VARCHAR(10) PRIMARY KEY,
    TenLLCT NVARCHAR(50) NOT NULL
);

INSERT INTO TrinhDoLLCT VALUES
('SC', N'Sơ cấp'),
('TC', N'Trung cấp'),
('CC', N'Cao cấp');

CREATE TABLE CapBac (
    MaCapBac VARCHAR(10) PRIMARY KEY,
...
('CV',  N'Chuyên viên'),
('TP',  N'Trưởng phòng'),
('PP',  N'Phó trưởng phòng'),
('CT',  N'Cục trưởng'),
('PCT', N'Phó Cục trưởng');
GO
...
CREATE TABLE GiangVien (
    ...
    TonGiao NVARCHAR(50),
    SDT VARCHAR(15),
    TrinhDoChuyenMon VARCHAR(10) NULL,
    TrinhDoLLCT VARCHAR(10) NULL,
    DonViCongTac NVARCHAR(100) NOT NULL,
    ChucVu VARCHAR(10) NULL,
    CapBac VARCHAR(10) NULL,
    ...
    CONSTRAINT FK_GV_TDCM FOREIGN KEY (TrinhDoChuyenMon) REFERENCES TrinhDoChuyenMon(MaTD),
    CONSTRAINT FK_GV_LLCT FOREIGN KEY (TrinhDoLLCT) REFERENCES TrinhDoLLCT(MaLLCT),
    CONSTRAINT FK_GV_CapBac FOREIGN KEY (CapBac) REFERENCES CapBac(MaCapBac),
    CONSTRAINT FK_GV_HocHam FOREIGN KEY (HocHam) REFERENCES HocHam(MaHocHam),
    CONSTRAINT FK_GV_HocVi FOREIGN KEY (HocVi) REFERENCES HocVi(MaHocVi),
    CONSTRAINT FK_GV_ChucVu FOREIGN KEY (ChucVu) REFERENCES ChucVu(MaChucVu)
);
...
CREATE TABLE DeTaiNCKH (
    ...
    CONSTRAINT FK_DeTai_CapNhiemVu
        FOREIGN KEY (CapNhiemVu)
        REFERENCES CapNhiemVu(MaCapNhiemVu)
);
GO
...
SELECT 
    ...
    ThoiHanHoanThanh,
    CASE 
        WHEN TinhTrang = N'Hoàn thành' THEN N'Hoàn thành'
        WHEN GETDATE() < ThoiGianBatDau THEN N'Chưa triển khai'
        WHEN GETDATE() BETWEEN ThoiGianBatDau AND ThoiHanHoanThanh 
...
WHERE GETDATE() > nv.ThoiHanHoanThanh
  AND nv.TinhTrang <> N'Hoàn thành';
GO
...
CREATE TABLE ThanhVienHoiDong (
    ...
    CONSTRAINT CK_ChucDanhHoiDong
        CHECK (ChucDanhHoiDong IN (
         N'Chủ tịch',
         N'Phó chủ tịch',
         N'Thư ký',
         N'Phản biện',
...
HAVING 
    COUNT(DISTINCT tv.MaCB) = 5
    AND SUM(CASE WHEN gv.HocHam = 'PGS' THEN 1 ELSE 0 END) >= 2
    AND SUM(CASE WHEN gv.ChucVu IN ('CT', 'PCT') THEN 1 ELSE 0 END) >= 1
    AND SUM(CASE WHEN gv.HocVi = 'HV_TS' THEN 1 ELSE 0 END) >= 1;
GO
...
CREATE TABLE GiaoTrinh (
    ...
    CapDoDaoTao VARCHAR(10) NOT NULL,
    Keywords NVARCHAR(200),
    TongGioKhoaHoc DECIMAL(10,2),

    CONSTRAINT FK_GiaoTrinh_CapDo
        FOREIGN KEY (CapDoDaoTao)
        REFERENCES CapDoDaoTao(MaCapDo)
);
...
-- 12. Công bố khoa học
CREATE TABLE LoaiHinhCongBo (
    MaLoai VARCHAR(10) PRIMARY KEY,
    TenLoai NVARCHAR(100) NOT NULL
);

INSERT INTO LoaiHinhCongBo VALUES
('TC',   N'Tạp chí'),
('HN',   N'Hội nghị'),
('CK',   N'Chuyên khảo'),
('SA',   N'Sách'),
('GT',   N'Giáo trình'),
('BC',   N'Báo cáo khoa học'),
('KL',   N'Kỷ yếu hội thảo');
...
CREATE TABLE CongBoKhoaHoc (
    ...
    CONSTRAINT FK_CongBo_LoaiHinh
        FOREIGN KEY (LoaiHinh)
        REFERENCES LoaiHinhCongBo(MaLoai)
);
